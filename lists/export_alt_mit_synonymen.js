...
function(head, req) {
  ...
  while (row = getRow ()) {
    ...
    if (row.key[1] === 0) {
      ...
      // für das alt sollen alle Daten aus den gewünschten Artgruppen
      // gewählt werden, also keinen Filter übernehmen

      // Exportobjekte um das Objekt ergänzen
      // der letzte Parameter 'alt' teilt mit, dass der Export für das Artenlistentool
      // erstellt wird und die Pflichtfelder benötigt
      exportObjekte = ergaenzeExportobjekteUmExportobjekt(
        objekt,
        üVar.felder,
        üVar.bezInZeilen,
        üVar.fasseTaxonomienZusammen,
        üVar.filterkriterien,
        exportObjekte,
        'alt'
      )
